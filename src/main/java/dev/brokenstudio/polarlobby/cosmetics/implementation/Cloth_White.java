...
public class Cloth_White extends AbstractCosmetic {

    public Cloth_White() {
        super("Weiß Kleidung", "Weiß wie der Schnee", CosmeticType.CLOTH, new PolarItem(Material.LEATHER_CHESTPLATE).color(Color.WHITE), Rarity.RARE);
    }
    ...
    public void select(Player player) {
        ...
        player.getInventory().setChestplate(new PolarItem(Material.LEATHER_CHESTPLATE).color(Color.WHITE).name("§8•● " + getRarity().color() + getName() + " §8●•"));
        player.getInventory().setLeggings(new PolarItem(Material.LEATHER_LEGGINGS).color(Color.WHITE).name("§8•● " + getRarity().color() + getName() + " §8●•"));
        player.getInventory().setBoots(new PolarItem(Material.LEATHER_BOOTS).color(Color.WHITE).name("§8•● " + getRarity().color() + getName() + " §8●•"));
    }
