...
public class Particle_Purple extends AbstractCosmetic {

    private LinkedList<Item> spawned;

    public Particle_Purple() {
        super("Lila Spur", "Du magst Lila, ich auch!", CosmeticType.PARTICLES, new PolarItem(Material.PURPLE_DYE), Rarity.ULTIMATE);
    ...
    public void use(Player player) {
        ...
        Item one = location.getWorld().dropItem(location,new ItemStack(Material.PURPLE_DYE));
        Item two = location.getWorld().dropItem(location,new ItemStack(Material.PURPLE_STAINED_GLASS));
        Item three = location.getWorld().dropItem(location,new ItemStack(Material.PURPLE_WOOL));
        Item four = location.getWorld().dropItem(location,new ItemStack(Material.PURPLE_CONCRETE));
